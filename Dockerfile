...
RUN /bin/bash -c 'source $HOME/.bashrc && \
 ...
 mkdir build && \
 cd build && \
 cmake -DCHECKPOINT_BUILD_TESTS:BOOL=1 -DCMAKE_INSTALL_PREFIX=$CHECKPOINT_BUILD/install -DCMAKE_CXX_COMPILER=$CXX -DCMAKE_C_COMPILER=$CC -Ddetector_DIR=$DETECTOR_BUILD/install $CHECKPOINT && \
 make && \
 make test && \
 make install'
